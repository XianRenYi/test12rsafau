...
const initMap = async () => {
  try {
    ...
    map.value.on('click', (e) => {
      ...
      alert('地图点击成功！坐标: ' + e.lnglat.lng + ', ' + e.lnglat.lat);
    });
    
    // 添加地图拖拽事件
    map.value.on('dragstart', (e) => {
      console.log('地图开始拖拽:', e);
    });
    
    map.value.on('dragend', (e) => {
      console.log('地图拖拽结束:', e);
    });
    
    // 添加地图缩放事件
    map.value.on('zoomchange', (e) => {
    ...
    if (mapContainer) {
      ...
      mapContainer.addEventListener('mousedown', (e) => {
        console.log('地图容器鼠标按下事件:', e);
      });
      
      mapContainer.addEventListener('mousemove', (e) => {
        console.log('地图容器鼠标移动事件:', e);
      });
...
.map-container {
  ...
  width: 100%;
  height: 100%;
  z-index: 1000; /* 提高地图容器的层级 */
  pointer-events: auto; /* 确保地图容器可以接收鼠标事件 */
...
.map-controls {
  ...
  top: 5px;
  left: 5px;
  z-index: 1000; /* 提高控制面板的层级，确保在地图之上 */
  pointer-events: auto; /* 确保控制面板可以交互 */
}
...
.amap_info {
  ...
  pointer-events: auto; /* 确保信息面板可以交互 */
}
