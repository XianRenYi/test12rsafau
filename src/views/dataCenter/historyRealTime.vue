            ...
            >
              ...
              >
                ...
                ></el-option>
                         ...
                         <el-form-item>
               <el-button
                 :type="isAutoRefresh ? 'danger' : 'success'"
...
const devQueryParams = reactive({
  ...
  identifiers: [],
  dayDaterange: [
    formatDateTime(new Date(Date.now() - 2 * 60 * 1000)), // 近2分钟
...
// 获取设备图表数据（不显示加载遮盖层）
const getDevChartDatasWithoutLoading = async () => {
  if (!devQueryParams.deviceId || !devQueryParams.identifiers.length) return;
...
const startAutoRefresh = () => {
  if (!devQueryParams.deviceId || !devQueryParams.identifiers.length) {
    ElMessage.warning('请先选择设备和变量');
    return;
  }
  ...
  ElMessage.success('自动刷新已开启，每秒更新一次数据');
};
...
const refreshData = async () => {
  ...
  try {
    ...
    const startTime = new Date(Date.now() - 60 * 1000); // 最近1分钟
...
const refreshDataWithoutLoading = async () => {
  ...
  try {
    ...
    // 直接调用数据获取，不显示遮盖层
    await getDevChartDatasWithoutLoading();
