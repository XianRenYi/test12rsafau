    ...
    <el-card style="margin: 6px; padding-bottom: 100px">
      ...
      >
            ...
            <el-row :gutter="100">
              <el-col :xs="24" :sm="24" :md="24" :lg="12" :xl="8">
                ...
                <el-form-item v-if="openServerTip">
                  <el-alert
                    type="info"
                    show-icon
                    description="当前选择TCP协议,设备编号生成为HEX格式"
                  ></el-alert>
                </el-form-item>
                <el-form-item v-if="openTip">
                  <el-alert
                    type="success"
                    show-icon
                    description="当前选择的产品属于modbus协议,将在网关设备创建后根据采集点模板生成子设备"
                  ></el-alert>
                </el-form-item>
              ...
              <el-col :xs="24" :sm="24" :md="24" :lg="12" :xl="8">
                ...
                <el-form-item label="其他信息" prop="remark" v-if="form.deviceId != 0">
                  ...
                  />
                  <el-button @click="handleViewMqtt()">认证信息</el-button>
                </el-form-item>
              </el-col>
              <el-col
                :xs="24"
        ...
        <el-tab-pane
          name="runningStatus"
          key="1"
          v-if="form.deviceType !== 3 && form.deviceId != 0 && !isSubDev"
        >
          <template #label>运行状态</template>
          <runningStatus
        ...
        <el-tab-pane
          name="deviceLog"
          key="5"
          v-if="form.deviceType !== 3 && form.deviceId != 0"
        >
          <template #label>事件日志</template>
          <deviceLog ref="deviceLogRef" :device="form" />
        </el-tab-pane>
        ...
        <el-tab-pane
          ...
          lazy
        >
          <template #label>实时统计</template>
          <deviceVariable ref="deviceVariableRef" :device="form" />
        </el-tab-pane>
        ...
        <el-tab-pane
          ...
          v-if="form.deviceType !== 3"
        >
          <template #label>图表统计</template>
          <deviceStatistic ref="deviceStatisticRef" :device="form" />
        </el-tab-pane>

        <!-- <el-tab-pane name="inlineVideo" :disabled="form.deviceId == 0" v-if="form.deviceType !== 3" lazy>
      <template #label>视频监控</template>          
...
// const dict = useDict(['iot_device_status', 'iot_location_way']);
      ...
      if (form.deviceId != 0) {
        ...
        updateDevice(updateData).then((response) => {
          if (response.data == 0) {
            ...
            Object.assign(form, JSON.parse(JSON.stringify(form)));
          }
        });
      } else {
