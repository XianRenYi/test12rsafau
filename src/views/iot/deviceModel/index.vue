  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="模型类别" prop="type">
        <el-select v-model="queryParams.type" placeholder="请选择模型类别" clearable style="width: 120px;">
...
function submitForm() {
  proxy.$refs["TemplateRef"].validate(valid => {
    ...
    // 验证对象或对象数组中的参数不能为空
    if (form.value.datatype    == 'object' || (form.value.datatype == 'array' && form.value.datatype == 'object')) {
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除通用物模型编号为"' + row.templateName + '"的数据项？').then(function() {
