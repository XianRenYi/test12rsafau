...
const formatDate = (date) => {
  ...
  const hours = String((d.getUTCHours() + 8) % 24).padStart(2, "0"); // 直接加8小时转为北京时间
...
const getDevIdentifierList = async (deviceId) => {
  try {
    ...
    listThingsModel(params).then((data) => {
      if (data.code === 200) {
        devIdentifierList.value = data.rows;
      }
    });
  } catch (error) {
...
const getDevChartDatas = async () => {
  ...
  try {
    ...
    const params = {
      ...
      beginTime: formatDate(devQueryParams.dayDaterange[0]),
      endTime: formatDate(devQueryParams.dayDaterange[1]),
    };
...
const handleDevResetQuery = () => {
  ...
  devQueryParams.dayDaterange = [
    new Date(new Date().getTime() - 3600 * 1000 * 2),
    new Date(),
  ];
  devDatas.value = [];
  devTotal.value = 0;
...
const getSceneChartDatas = async () => {
  ...
  try {
    ...
    const params = {
      ...
      ids: ids,
      beginTime: formatDate(sceneQueryParams.dayDaterange[0]),
      endTime: formatDate(sceneQueryParams.dayDaterange[1]),
    };
